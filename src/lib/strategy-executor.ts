    ...
    try {
      if (config.type === "SNIPER") {
        ...
        results.push({
          ...
          details: `Strategy type ${config.type} not yet implemented`,
        });
      }
    } catch (error) {
      results.push({
...
async function executeSniperStrategy(
  ...
  const targetPair = newPairs.find((pair) => !boughtTokens.has(pair.address));
  ...
  if (!targetPair) {
    return {
      ...
      details: `Already bought all ${newPairs.length} matching pairs`,
    };
  }

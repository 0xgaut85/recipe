...
export const WithdrawModal: FC<WithdrawModalProps> = ({
      ...
      toast.success(
        <div className="flex flex-col gap-1">
          ...
          <a
            ...
            className="text-accent-pink hover:underline flex items-center gap-1"
          >
  ...
  // The amount entered IS what the user receives - fees are deducted from remaining balance
          ...
          >
            ...
            {withdrawInfo ? (
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                {/* Balance Display */}
                <div className="p-4 rounded-xl bg-accent-blue/10 border-2 border-accent-blue/20">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-ink/60 lowercase">available balance</span>
                    <span className="font-display text-xl text-ink">
                      {withdrawInfo.solBalance.toFixed(4)} SOL
                    </span>
                  </div>
                  <div className="flex justify-between items-center mt-1">
                    <span className="text-xs text-ink/40 lowercase">max withdrawable</span>
                ...
                {/* Deposit Address (for reference) */}
                <div className="p-3 rounded-xl bg-ink/5 border border-ink/10">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-ink/40 lowercase">your deposit address</span>
                    <button
                      type="button"
                      onClick={copyAddress}
                      className="text-ink/60 hover:text-accent-pink transition-colors"
                    >
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-ink lowercase">
                    amount (sol)
                  </label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full p-3 pr-16 rounded-xl border-2 border-ink/20 focus:border-accent-pink focus:outline-none transition-colors"
                    />
                    <button
                      type="button"
                      onClick={handleMaxClick}
                      className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-1 rounded-full bg-accent-pink/10 text-accent-pink text-sm font-semibold hover:bg-accent-pink/20 transition-colors"
                    >
                      max
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className={`w-full p-3 rounded-xl border-2 focus:outline-none transition-colors ${
                      isValidAddress === null
                        ? "border-ink/20 focus:border-accent-pink"
                        : isValidAddress
                ...
                {/* Fee Breakdown */}
                <div className="p-4 rounded-xl bg-ink/5 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-ink/60 lowercase">network fee</span>
                    <span className="text-ink">{withdrawInfo.networkFee} SOL</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-ink/60 lowercase">platform fee</span>
                    <span className="text-ink">{withdrawInfo.platformFee} SOL</span>
                  </div>
                  <div className="flex justify-between text-sm text-ink/40">
                    <span className="lowercase">fees deducted from balance</span>
                    <span>{withdrawInfo.totalFee} SOL</span>
                  </div>
                  <div className="border-t border-ink/10 pt-2 flex justify-between">
                    <span className="font-semibold text-ink lowercase">you receive</span>
                    <span className={`font-semibold ${isValidAmount ? "text-green-600" : "text-ink"}`}>
                ...
                <motion.button
                  ...
                  className="w-full p-4 rounded-full bg-ink text-white font-semibold lowercase flex items-center justify-center gap-2 hover:bg-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  whileTap={{ scale: 0.98 }}
                >
                  <Send size={18} />

 ...
 * Get token decimals
 */
export function getTokenDecimals(mint) {
 ...
 */
export async function getQuote(inputToken, outputToken, amount, slippageBps = 50) {
 ...
 */
async function confirmTransactionPolling(connection, signature, blockhash, maxRetries = 30, retryDelayMs = 1000) {
 ...
 */
export async function resolveTokenMintWithSearch(token) {
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            const exactMatch = results.find((r) => r.symbol.toUpperCase() === upperToken ||

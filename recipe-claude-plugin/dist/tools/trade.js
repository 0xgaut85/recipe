 ...
 */
import { getQuote, executeSwap, listTokens, resolveTokenMintWithSearch, } from "../lib/jupiter.js";
    ...
    switch (name) {
        case "claude_trade_swap_quote": {
            ...
            try {
                ...
                const quote = await getQuote(inputToken.address, outputToken.address, amount, slippageValue);
                return {
                    content: [
                        {
                            type: "text",
        ...
        case "claude_trade_swap_execute": {
            ...
            const amount = args?.amount;
            const slippageBps = args?.slippageBps;
            const slippage = args?.slippage || 1; // Default 1% for memecoins
            if (!inputTokenArg || !outputTokenArg || !amount) {
                return {
                    content: [
                        {
            ...
            try {
                ...
                const slippageValue = slippageBps || Math.floor(slippage * 100);
                const result = await executeSwap(inputToken.address, outputToken.address, amount, slippageValue);
                return {
                    content: [
                        {
                            type: "text",
        ...
        case "claude_trade_quick_buy": {
            ...
            try {
                ...
                const result = await executeSwap("SOL", outputToken.address, solAmount, slippageBps);
                return {
                    content: [
                        {
                            type: "text",
        ...
        case "claude_trade_quick_sell": {
            ...
            try {
                ...
                const result = await executeSwap(inputToken.address, "SOL", amount, slippageBps);
                return {
                    content: [
                        {
                            type: "text",

...
export async function handleStrategyTool(name, args) {
    switch (name) {
        case "claude_trade_strategy_create": {
            ...
            let config = {
                type: strategyType,
                amount: args.amount,
                slippageBps: args.slippageBps || (strategyType === "SNIPER" ? 300 : 50),
            };
        ...
        case "claude_trade_strategy_list": {
            ...
            if (typeFilter === "all") {
                ...
                examples = [STRATEGY_EXAMPLES[typeFilter]];
            }
            else {
                return {
                    content: [
        ...
        case "claude_trade_strategy_validate": {
            ...
            if (strategy.type === "SPOT") {
                if (!strategy.inputToken)
                    issues.push("SPOT: Missing inputToken");
                if (!strategy.outputToken)
                    issues.push("SPOT: Missing outputToken");
            }
            if (strategy.type === "CONDITIONAL") {
                if (!strategy.inputToken)
                    issues.push("CONDITIONAL: Missing inputToken");
                if (!strategy.condition)
                    issues.push("CONDITIONAL: Missing condition object");
                else {
                    const condition = strategy.condition;
                    if (!condition.indicator)
                        issues.push("CONDITIONAL: Missing condition.indicator");
                    if (!condition.trigger)
                        issues.push("CONDITIONAL: Missing condition.trigger");
                }
            }
            // Safety warnings
            if (typeof strategy.amount === "number" && strategy.amount > 10) {
            ...
            if (typeof strategy.slippageBps === "number" && strategy.slippageBps > 500) {
